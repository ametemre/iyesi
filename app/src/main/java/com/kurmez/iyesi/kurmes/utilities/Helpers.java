...
public class Helpers {
    ...
    public static List<Iyesi> parseProfiles(String jsonBody) throws JSONException {
        JSONObject root = new JSONObject(jsonBody);
        JSONArray users = root.optJSONArray("users");
        List<Iyesi> list = new ArrayList<>();
        ...
        for (int i = 0; i < users.length(); i++) {
            ...
            list.add(new Iyesi(uid, username, email, location, phone, role, avatar_url));
        }
        return list;
    }
